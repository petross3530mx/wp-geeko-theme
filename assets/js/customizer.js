jQuery(function ($) {
	...
	$.extend(Customizer.prototype, {
		...
		init: function (el) {
			...
			this.$iframe
				...
				.on(
					'load',
					function () {
						// Re-apply unsaved CSS variable changes after iframe is loaded if necessary.
		...
		apply: function (mode) {
			...
			return $.post(ajaxurl, params, null, 'json').done(function (json) {
				if (json.success) {
					if ('temp' !== mode) {
		...
		/**
		 * Reload preview and re-apply any CSS variable changes made in the current session.
		 * This method is useful after we have updated the backend setting and want the preview
		 * to reflects the changes accordingly.
		 */
		reload: function () {
			// Function to reload the iframe.
			var iframeReload = $.proxy(function () {
				this.$iframe[0].contentWindow.location.reload(true);
			}, this);

			setTimeout(iframeReload, 700);
		},
		...
		updateButtons: _.debounce(function () {
			...
			if ((this.modified && !this.readonly) || this.publishedPreset !== this.selectedPreset) {
				this.onbeforeunload = this.onbeforeunload || window.onbeforeunload;
				window.onbeforeunload = function () {
					return TEXT_NOTICE_RELOAD;
				};
			} else {
				if (typeof this.onbeforeunload !== 'undefined') {
		...
		onMainMore: function (e) {
			var $btn = this.$btnMainMore,
				$dropdown = $btn.closest('.gc-dropdown').find('.gc-dropdown__box');

			$(document).off('click.gc-customizer-main-more');
			...
			if ($dropdown.is(':visible')) {
				...
				$dropdown.show();
				setTimeout(function () {
					$(document).on('click.gc-customizer-main-more', function () {
						$(document).off('click.gc-customizer-main-more');
						$dropdown.hide();
					});
				}, 100);
		...
		onMore: function (e) {
			...
			$(document).off('click.gc-customizer-more');
			...
			if ($dropdown.is(':visible')) {
				...
				$dropdown.show();
				setTimeout(function () {
					$(document).on('click.gc-customizer-more', function () {
						$(document).off('click.gc-customizer-more');
						$dropdown.hide();
					});
				}, 100);
		...
		onTab: function (e, tab) {
			...
			this.$btnBack.removeAttr('disabled');
